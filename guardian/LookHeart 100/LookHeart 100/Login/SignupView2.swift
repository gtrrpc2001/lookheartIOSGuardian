...
class SignupView2 : UIViewController {
...
//    }()
    ...
    lazy var agreeButton: UIButton = {
        ...
        UserDefaults.standard.set(false, forKey: "secondAgree")
    ...
    @objc func autoLoginTapped(_ sender: UIButton) {
        if(UserDefaults.standard.bool(forKey: "secondAgree")){
            UserDefaults.standard.set(false, forKey: "secondAgree")
            ...
            UserDefaults.standard.set(true, forKey: "secondAgree")
...
//        agreeTextView.text = privacyTxt
        ...
        DispatchQueue.global().async { [self] in
            ...
            DispatchQueue.main.async {
                ...
                self.agreeTextView.text = self.privacyTxt
            }
        }
        setup()
        Constraints()
    ...
    // Text 파일 읽기
    func readTxtfile() {
        let paths = Bundle.main.path(forResource: "privacy.txt", ofType: nil)
        guard paths != nil else { return }
        
        do {
            privacyTxt = try String(contentsOfFile: paths!, encoding: .utf8)
        }
